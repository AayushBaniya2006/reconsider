...
## Landing site (2026-08-14) — Intercom-style front door
- **The "One Pager Link" is now a landing page** (per user request, styled after Intercom hero): `docs/index.html` — bold Archivo headline "Appeal the score. Argue the statute. Know when not to file.", wildfire-dusk hero (CSS-only: ember-glow gradient + topo contours + ember specks), California-poppy accent, floating product shot, sections answering the 3 judging questions + how-it-works + stats + evidence-in-flight. Shared `docs/style.css`.
- **`docs/signin.html`** — real-looking sign-in that SKIPS to the app (no OAuth): email/password (non-functional) + ember "Skip sign-in — enter the demo →" → app.html.
- **`docs/app.html`** — the signed-in workspace = the legal-editorial case dashboard + dark app-bar ("signed in as producer@demo"). Generated by `site.ts` (now emits both out/index.html for CLI demo AND docs/app.html for the site; copies packet PDFs → docs/assets/). Hero product shot: docs/assets/product-shot.png.
- Design intent: landing MARKETS the agent; the AGENT is still the CLI (keeps "build an agent not a website with a map" true). Continuity: poppy accent carries from landing → app.
- Verified LIVE: landing/signin/app/style.css/product-shot/packet-PDF all HTTP 200 on Pages; full click-flow works. E2E now 11 checks (added landing, sign-in skip-target, app view); unit 9. Both green.
- Fonts: Archivo (display/body) + IBM Plex Mono (labels) on the site; app keeps Libre Caslon + Plex Mono.
